...
Hooks are the larger half of praxis: **112 hooks**, registered at 137 points across
...
| ------ | ------- | -------------- |
| `preflight-gate` | 40 | Inspects a tool call before it runs and can deny it |
| `completion-verify` | 18 | Fires at `Stop`; selected hooks also run at `SubagentStop` — can block a response that claims completion without evidence |
| `advisory-nudge` | 49 | Prints a warning to stderr and lets the call through — 19 read a `PRAXIS_*_STRICT` variable that makes them stop the call instead |
...
**One gate.** 74 of the 112 hooks declare an opt-out or tuning variable. Which variable
...
`gemini:flash`). Bare names (`fable`, `opus`, `sonnet`, `haiku`) always resolve to
