...
기본 대기는 540초이고 `--timeout <초>` 로 조정합니다. 위임자는 폴링하지 않고
Step 7 에서 질문을 보므로, 짧은 대기는 정상 경로에서 타임아웃이 납니다.

대기값은 워커를 돌리는 하네스의 호출 상한보다 **낮아야** 합니다. `ask` 는 판정을
대기가 끝나는 시점에 stdout 으로 한 번 씁니다. 상한이 같거나 더 낮으면 그 write
가 호출 종료에 잘려나가고, 워커는 `verdict='timeout'` 대신 신호 종료만 봅니다 —
`timeout` 행이 발화하지 못하고, 워커는 "판정 없음"과 "게이트 고장"을 구분할 수
없게 됩니다. Claude Code 의 Bash 도구 상한이 600초라 기본값을 540 으로 둡니다.
...
질문을 아무도 답하지 않은 채 통과시킵니다. 두 사람이 같은 질문에 동시에 답해도
마찬가지입니다: 먼저 잡은 쪽만 기록하고 신호하며, 나중 쪽은 실제로 확정된 판정을
그대로 돌려받습니다. 드물게 `state='pending'` 이 나오면 다른 `resolve` 가 아직
진행 중이라는 뜻이므로 다시 조회하세요.
