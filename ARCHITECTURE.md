...
| `trino-catalog-gate` | PreToolUse | Block Trino MCP queries with 3-part catalog references when `SHOW CATALOGS` has not run in this session; bypass via inline `-- catalog-enumerated: verified` or `PRAXIS_TRINO_CATALOG_GATE=skip` (issue #321) | [docs/hook/trino-catalog-gate.md](docs/hook/trino-catalog-gate.md) |
