...
| `block-personal-asset-leak` | PreToolUse | Advisory nudge (opt-in strict: block via `PRAXIS_PERSONAL_LEAK_STRICT=1`) when a `gh issue/pr` write body contains an absolute home-dotfiles path (`/Users/<name>/.claude/...`, `/home/<name>/.config/...`) — deterministic backstop for literal personal-asset path leaks; tilde `~/` form and `/projects/` worktree paths excluded; semantic surfacing out of scope (issue #563) | [hooks/advisory-nudge/block-personal-asset-leak/spec.md](hooks/advisory-nudge/block-personal-asset-leak/spec.md) |
