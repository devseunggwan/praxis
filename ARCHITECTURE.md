...
| `verdict-gap-coexistence-gate` | Stop | Advisory (non-blocking) when a GO-verdict phrase and an unresolved-gap marker coexist anywhere in the same output with no conditional linkage between them; silent when the GO phrase is negated, when the gap marker is followed by resolution language, or when the recommended conditional wording is already present; strict via `PRAXIS_VERDICT_GAP_STRICT`, bypass via `PRAXIS_VERDICT_GAP_BYPASS` (issue #845) | [hooks/completion-verify/verdict-gap-coexistence-gate/spec.md](hooks/completion-verify/verdict-gap-coexistence-gate/spec.md) |
