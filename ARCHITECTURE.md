...
| `side-effect-scan` | PreToolUse | Tiered gate on commands with collateral side effects. **ask**: `git push`, `gh pr merge/create`, `gh workflow run`, `kubectl apply/delete/replace/patch`, and the `wrapper-commit` category. **advise** (stderr, no prompt — issue #874): `git commit/merge/rebase/cherry-pick/revert`, local-only and reversible from the same shell (`git reset --hard ORIG_HEAD` / `HEAD@{1}`) — publication is `git push`, which stays an ask. Reversibility is the whole ground; the sibling commit gates are context, not justification (issue #1153). A mixed match stays an ask and names every matched category | [hooks/preflight-gate/side-effect-scan/spec.md](hooks/preflight-gate/side-effect-scan/spec.md) |
