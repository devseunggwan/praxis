...
| `menu-mutation-tier-advisory` | PreToolUse | Advisory (opt-in strict via `PRAXIS_MENU_MUTATION_TIER_STRICT=1`) on `AskUserQuestion` when every candidate option in a question carries a mutation signal and none carries a non-mutating one (preview / dev / sandbox / dry-run / `보고만`) — the menu then asks "how much", never "whether". An option that merely declines to act (`다음 정기 실행에 맡김`) is classified as abandonment: it neither suppresses the advisory nor counts as a candidate (issue #963) | [hooks/advisory-nudge/menu-mutation-tier-advisory/spec.md](hooks/advisory-nudge/menu-mutation-tier-advisory/spec.md) |
