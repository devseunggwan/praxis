...
| `post-compact-context-inject` | UserPromptSubmit | Inject session_id / worktree / branch / open PR / strike state via `additionalContext` on the FIRST prompt after a `/compact`; tail-scans transcript JSONL for `isCompactSummary == true`, session-scoped dedup, fail-open (issue #472, supersedes #466) | [hooks/advisory-nudge/post-compact-context-inject/spec.md](hooks/advisory-nudge/post-compact-context-inject/spec.md) |
