...
| `pr-claim-mutation-gate` | Stop | Advisory (non-blocking) when the final message pairs a PR/review subject with a processed-claim verb (처리했/반영했/resolved/applied) and the current turn contains no PR-surface mutation (`git push`, `gh pr comment/review`, write-method `gh api`, `resolveReviewThread`); a read-only `gh api` listing does not clear it (issue #868) | [hooks/completion-verify/pr-claim-mutation-gate/spec.md](hooks/completion-verify/pr-claim-mutation-gate/spec.md) |
