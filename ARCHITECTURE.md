...
| `pre-commit-staged-file-enumeration` | PreToolUse | Advisory nudge before `git commit` listing staged file additions not created via Write/Edit/NotebookEdit this session — heredoc / `> file` / external-script output caught before it rides into the commit (issue #784) | [hooks/advisory-nudge/pre-commit-staged-file-enumeration/spec.md](hooks/advisory-nudge/pre-commit-staged-file-enumeration/spec.md) |
