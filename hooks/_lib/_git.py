...
(issue #470) extraction. Contract:
  ...
  - Timeouts default to 5s; sites whose hand-rolled copy used a different
    budget (2s, 3s, deadline-derived) pass `timeout=` explicitly to preserve
    their behavior.
...
def run_git(
    ...
    invalid cwd, embedded-null argv. Never raises.
    """
    try:
...
def repo_root(cwd: Optional[str] = None, timeout: float = 5) -> Optional[str]:
    """Return the git repo root for cwd, or None if not in a repo / on error."""
    out = run_git(["rev-parse", "--show-toplevel"], timeout=timeout, cwd=cwd)
    if out is None:
        return None
    root = out.strip()
    return root or None
...
def origin_slug(cwd: Optional[str] = None, timeout: float = 5) -> Optional[str]:
    ...
    """
    out = run_git(["remote", "get-url", "origin"], timeout=timeout, cwd=cwd)
    if out is None:
        return None
    url = out.strip()
    if not url:
