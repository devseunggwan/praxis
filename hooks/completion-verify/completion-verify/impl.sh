...
# Each alternative swallows the claim word itself — deleting only the negator
# would leave "done" behind and manufacture the claim it was meant to drop.
# Deliberately narrow: only a negator or modal sitting IMMEDIATELY before the
# claim word is stripped, because the negation has to govern that word. So
# "done, but the tests are not finished" stays a claim (the negation trails it),
# and "not all tests are done" also stays one (a quantifier intervenes) — the
# gate would rather fire on a non-claim than go silent on a real one.
# The typographic apostrophe is spelled as an escape: a literal one in source
# reads as a stray quote to shell linters.
NON_CLAIM_APOS=$(printf '\342\200\231')
NON_CLAIM_FORMS="(미|예상 ?)완료|(not|n[${NON_CLAIM_APOS}']t|never)( yet)?( be| been)? (done|finished|complete)|(will|would|should|can|could|may|might|to|going to) (be|get) (done|finished|complete)|expect(ed|s|ing)? to (finish|complete|be done|be finished)"
...
# `I` keeps the stripping case-insensitive, matching the `grep -iE` below —
# without it "NOT DONE:" would survive into the claim scan.
LAST_LINES=$(printf '%s\n' "$LAST_TEXT" | tail -10 | sed -E "s/${NON_CLAIM_FORMS}//gI")
