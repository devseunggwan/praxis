...
Limitations:
  ...
  - Shared-tokenizer detection boundaries (inherited from the shared
    `safe_tokenize` / `iter_command_starts` pipeline): `result=$(git commit ...)`
    is missed; partial commits (`--only` / pathspec) over-list all staged
    additions. The
