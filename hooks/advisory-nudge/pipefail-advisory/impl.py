...
Delivery channels (issues #874, #1408):
  ...
  Every advisory is written twice, unconditionally.

  stdout carries it as `hookSpecificOutput.additionalContext`, the one
  exit-0 PreToolUse channel that reaches the model — stderr at exit 0 reaches
  only the debug log (canary #841). #874 added this copy as an env-gated
  experiment arm, and with the arm off the hook detected correctly while
  telling the actor nothing: #1408 records one session assembling the same
  masked-exit merge 11 times with the hook firing on each.

  stderr keeps the same text because `_fire_ledger.classify_decision` grades
  a fire `advise` from non-empty stderr; without it every fire would be
  recorded as `pass`. `_dispatch.run_group` forwards member stderr and merges
  non-decision `additionalContext` into its own stdout, so both copies survive
  the dispatch group. See spec.md § "Delivery channels".
...
from _hook_io import emit_additional_context, emit_updated_input  # type: ignore[import-not-found]  # noqa: E402
...
def main() -> int:
    ...
    # stderr is what `_fire_ledger` grades `advise` on; stdout is what the
    # model reads. Neither replaces the other (issue #1408).
