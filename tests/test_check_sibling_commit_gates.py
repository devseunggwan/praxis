...
Three further groups cover the PR #1142 review findings:
  ...
  - **host scoping.** Three of the eight gated hooks carry `hosts: ["claude"]`,
    so the sibling set the runtime actually installs is 8 on `claude` and 5 on
  ...
  - **the second count.** "Four of the eight siblings are the checklist …" was
...
EXPECTED_PER_HOST = {"claude": 8, "codex": 5, "cursor": 5}
...
# `commit-decomposition-advisory` stands in as the fixture's "retired hook":
# it is `claude`-only, same as the hook this file used to exercise before
# issue #1439 removed it.
_RETIRED_HOOK_ENTRY = (
    '      "name": "commit-decomposition-advisory",\n'
    '      "role": "advisory-nudge",\n'
    '      "event": "PreToolUse",\n'
    '      "matcher": "Bash",\n'
)
_RETIRED_HOOK_GATES = '      "gates": [\n        "git-commit"\n      ],\n'
...
def test_name_missing_from_spec_table_is_drift(tmp_path):
    ...
    assert any(
        "commit-decomposition-advisory" in d and "missing from the enumeration" in d
        for d in drifts
    ), drifts
...
def test_name_missing_from_impl_docstring_is_drift(tmp_path):
    repo = _tree(tmp_path, {gates.IMPL: ("commit-decomposition-advisory, ", "")})
    drifts = gates.check(repo)
    assert any(
        "impl.py" in d
        and "commit-decomposition-advisory" in d
...
def test_retired_hook_left_on_the_surfaces_is_drift(tmp_path):
    ...
    repo = _tree(
        tmp_path,
        {gates.MANIFEST: (_RETIRED_HOOK_ENTRY + _RETIRED_HOOK_GATES, _RETIRED_HOOK_ENTRY)},
    )
    drifts = gates.check(repo)
    assert any(
        "commit-decomposition-advisory" in d and "carries no gates" in d
        for d in drifts
    ), drifts
...
def test_gate_label_on_a_hook_absent_from_disk_is_drift(tmp_path):
    repo = _tree(tmp_path)
    shutil.rmtree(repo / "hooks" / "advisory-nudge" / "commit-decomposition-advisory")
    drifts = gates.check(repo)
...
def test_host_table_stale_count_is_drift(tmp_path):
    ...
    repo = _tree(tmp_path, {gates.SPEC: ("| `codex` | 5 | 2 |", "| `codex` | 8 | 4 |")})
    drifts = gates.check(repo)
    assert any(
        "'codex' row says 8 sibling commit gates, manifest derives 5" in d
        for d in drifts
    ), drifts
...
def test_host_table_stale_checklist_count_is_drift(tmp_path):
    repo = _tree(
        tmp_path, {gates.SPEC: ("| `cursor` | 5 | 2 |", "| `cursor` | 5 | 4 |")}
    )
    drifts = gates.check(repo)
    assert any(
...
def test_missing_host_row_is_drift(tmp_path):
    repo = _tree(tmp_path, {gates.SPEC: ("| `cursor` | 5 | 2 |\n", "")})
    drifts = gates.check(repo)
...
def test_narrowing_a_hosts_whitelist_fails_both_host_surfaces(tmp_path):
    ...
    assert any(
        "'codex' row says 5 sibling commit gates, manifest derives 4" in d
        for d in drifts
    ), drifts
