...
def test_group_members_count_and_roles():
    ...
    assert len(members) == 39, f"expected 39 exact-Bash members, got {len(members)}"
    ...
    assert roles.count("preflight-gate") == 24
...
def test_group_members_host_filter():
    ...
    assert len(unfiltered) == 39  # host=None -> canonical, unfiltered view
