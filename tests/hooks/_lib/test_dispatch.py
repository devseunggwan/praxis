...
def test_group_members_count_and_roles():
    ...
    assert len(members) == 43, f"expected 43 exact-Bash members, got {len(members)}"
    ...
    assert roles.count("advisory-nudge") == 18
...
def test_group_members_host_filter():
    ...
    assert len(unfiltered) == 43  # host=None -> canonical, unfiltered view
