...
def test_group_members_count_and_roles():
    ...
    assert len(members) == 45, f"expected 45 exact-Bash members, got {len(members)}"
    ...
    assert roles.count("advisory-nudge") == 20
...
def test_group_members_host_filter():
    ...
    assert len(unfiltered) == 45  # host=None -> canonical, unfiltered view
