...
def test_group_members_count_and_roles():
    ...
    assert len(members) == 40, f"expected 40 exact-Bash members, got {len(members)}"
    ...
    assert roles.count("advisory-nudge") == 15
...
def test_group_members_host_filter():
    ...
    assert len(unfiltered) == 40  # host=None -> canonical, unfiltered view
