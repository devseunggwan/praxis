...
def test_group_members_count_and_roles():
    ...
    assert len(members) == 48, f"expected 48 exact-Bash members, got {len(members)}"
    ...
    assert roles.count("advisory-nudge") == 22
...
def test_group_members_host_filter():
    ...
    assert len(unfiltered) == 48  # host=None -> canonical, unfiltered view
