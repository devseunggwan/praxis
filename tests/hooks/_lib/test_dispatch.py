...
def test_group_members_count_and_roles():
    ...
    assert len(members) == 41, f"expected 41 exact-Bash members, got {len(members)}"
    ...
    assert roles.count("advisory-nudge") == 16
...
def test_group_members_host_filter():
    ...
    assert len(unfiltered) == 41  # host=None -> canonical, unfiltered view
