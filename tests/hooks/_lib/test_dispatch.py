...
def test_group_members_count_and_roles():
    ...
    assert len(members) == 44, f"expected 44 exact-Bash members, got {len(members)}"
    ...
    assert roles.count("advisory-nudge") == 19
...
def test_group_members_host_filter():
    ...
    assert len(unfiltered) == 44  # host=None -> canonical, unfiltered view
