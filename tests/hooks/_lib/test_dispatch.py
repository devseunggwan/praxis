...
def test_group_members_count_and_roles():
    ...
    assert len(members) == 47, f"expected 47 exact-Bash members, got {len(members)}"
    ...
    assert roles.count("advisory-nudge") == 21
...
def test_group_members_host_filter():
    ...
    assert len(unfiltered) == 47  # host=None -> canonical, unfiltered view
