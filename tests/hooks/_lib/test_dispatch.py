...
def test_group_members_count_and_roles():
    ...
    assert len(members) == 34, f"expected 34 exact-Bash members, got {len(members)}"
...
def test_group_members_host_filter():
    ...
    assert len(unfiltered) == 34  # host=None -> canonical, unfiltered view
    # the only host-restricted Bash members are the 3 claude-only guards
