...
def test_group_members_count_and_roles():
    ...
    assert len(members) == 35, f"expected 35 exact-Bash members, got {len(members)}"
    ...
    assert roles.count("advisory-nudge") == 13
...
def test_group_members_host_filter():
    ...
    assert len(unfiltered) == 35  # host=None -> canonical, unfiltered view
