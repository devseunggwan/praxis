...
def test_group_members_count_and_roles():
    ...
    assert len(members) == 42, f"expected 42 exact-Bash members, got {len(members)}"
    ...
    assert roles.count("preflight-gate") == 25
    assert roles.count("advisory-nudge") == 17
...
def test_group_members_host_filter():
    ...
    assert len(unfiltered) == 42  # host=None -> canonical, unfiltered view
