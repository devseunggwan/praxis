...
| [elapsed-time-signal](../../hooks/advisory-nudge/elapsed-time-signal/spec.md) | UserPromptSubmit + PostToolUse(`.*`) + PostToolUseFailure (claude only) | Opt-in (`PRAXIS_TIME_START_EPOCH`, set by `cmux-delegate --time-budget`): inject `elapsed <n>s / <budget>s` (or `elapsed <n>s`) as `additionalContext` on each message returned to a delegated worker, per the Opus 5.5 time-signal guidance; advisory only, no hard stop; POSIX sh so an env-less session exits before reading stdin — issue #1501 |
