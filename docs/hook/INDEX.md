...
| [zsh-word-split-advisory](../../hooks/advisory-nudge/zsh-word-split-advisory/spec.md) | PreToolUse | Advisory nudge when a Bash command hands an unquoted `$var` to `set --` or `for ... in` under zsh, where `SH_WORD_SPLIT` is off and the value arrives as one argument rather than several — opt out in-band with `# word-split:ok` (#1405) |
