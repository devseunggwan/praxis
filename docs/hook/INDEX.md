...
| [early-stop-advisory](../../hooks/completion-verify/early-stop-advisory/spec.md) | Stop | Advisory when the last assistant text ends the turn with requested work still open — a closing next-step announcement with no tool call (`다음 단계로 …하겠습니다` / `Next, I'll …`), a prose offer to continue on the user's preference (`원하시면 남은 …도 이어서` / `if you'd like, I can continue`), or an interim report listing unfinished items; silent on a named blocker, a user-requested report/plan/question, `stop_hook_active`, and type-3 menus owned by `prose-option-menu-advisory`; bypass `PRAXIS_EARLY_STOP_BYPASS` (#1498) |
