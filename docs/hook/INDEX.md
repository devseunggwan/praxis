...
| [elapsed-time-signal](../../hooks/advisory-nudge/elapsed-time-signal/spec.md) | UserPromptSubmit + PostToolUse(`.*`) + PostToolUseFailure (claude only) | Opt-in (`PRAXIS_TIME_START_EPOCH`, set by `cmux-delegate --time-budget`): inject `elapsed <n>s / <budget>s` (or `elapsed <n>s`) as `additionalContext` on each message returned to a delegated worker, adapting the Opus 5.5 guide's time signal for lead/team setups (per-worker and non-Opus-5.5 use untested); advisory only, no hard stop; POSIX sh so an env-less session exits before reading stdin — issue #1501 |
