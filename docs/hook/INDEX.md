...
| [rejected-call-probe-gate](../../hooks/preflight-gate/rejected-call-probe-gate/spec.md) | PreToolUse | Ask on the next mutating call after a refused or interrupted mutating call until a read-only probe of state (`git`/`gh`/`kubectl`/`aws`/`docker`, read-only MCP) has run, since a refused call may still have run (#1488) |
