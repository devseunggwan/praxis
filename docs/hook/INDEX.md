...
| [cross-tool-reroute-gate](../../hooks/preflight-gate/cross-tool-reroute-gate/spec.md) | PreToolUse | Ask before a call reaches, through a different tool, a SQL table or file a hook already blocked this session — quotes the original block; lifted per (block, new tool) only once a call on that route actually ran (#1485) |
| [question-turn-mutation-gate](../../hooks/preflight-gate/question-turn-mutation-gate/spec.md) | PreToolUse | Ask on every mutating call (Bash not provably read-only, Edit/Write/NotebookEdit, MCP write verb) in a turn whose opening human message reads as a question or a challenge; read-only probes stay silent, the next human message lifts it (#1486) |
| [rejected-call-probe-gate](../../hooks/preflight-gate/rejected-call-probe-gate/spec.md) | PreToolUse | Ask on the next mutating call after a refused or interrupted mutating call until read-only probes have covered every surface the refused call could have changed (local git, remote ref, GitHub, local files, the tool or MCP server it called), since a refused call may still have run (#1488) |
| [parallel-gated-mutation-gate](../../hooks/preflight-gate/parallel-gated-mutation-gate/spec.md) | PostToolBatch | Block when one parallel batch repeats a `gh` write on the same `(noun, verb, target)` key — the duplicate-issue shape (#98/#99) that no per-call event can see; closes the gap praxis had recorded as behaviour-rule-only — issue #1368 |
...
| [memory-distillation-fields-gate](../../hooks/preflight-gate/memory-distillation-fields-gate/spec.md) | PreToolUse | Block a `Write` to a memory entry whose frontmatter omits `recurrence` / `enforcement` / `escalated_to` under `metadata:`, or carries them flat at the top level — the promotion queue greps the nested form, so either shape drops the entry out of it silently; bypass `PRAXIS_HOOK_BYPASS_MEMORY_FIELDS` (#1405) |
...
| [zsh-dialect-advisory](../../hooks/advisory-nudge/zsh-dialect-advisory/spec.md) | PreToolUse | `ask` on three deterministic zsh shapes — a word starting with `=` (EQUALS expansion, so `[ "$x" == y ]` too), an unmatched `[` inside a `${...}` pattern operator, and a heredoc opener shadowed by its own delimiter — plus an advisory when an unquoted `$var` reaches `set --` / `for ... in`, where `SH_WORD_SPLIT` is off; opt out in-band with `# zsh-dialect:ok` or `# word-split:ok` (#1405, #1425) |
| [secret-print-redaction-advisory](../../hooks/advisory-nudge/secret-print-redaction-advisory/spec.md) | PreToolUse | Advisory nudge when a live Bash command or an agent-authored script (Write/Edit/heredoc/echo-append) both fetches a secret (`orgctl token fetch`, `aws secretsmanager get-secret-value`, `vault kv get`, ...) and routes the value to stdout unmasked (2-signal AND gate; masked/digest output and bare interactive fetches stay silent) — issue #827 |
...
| [negation-answer-quote-advisory](../../hooks/advisory-nudge/negation-answer-quote-advisory/spec.md) | PreToolUse | Soft `ask` when an external write follows a free-text `AskUserQuestion` answer that opens with a negation marker and nothing since has quoted that answer back (#1441) |
...
| [postcompact-context](../../hooks/advisory-nudge/postcompact-context/spec.md) | SessionStart (`compact`) | Inject session_id / cwd / branch / active PR / strike state as `additionalContext` on the session start Claude Code raises after a compaction — the event is the trigger, no transcript scan or dedup state; also re-injects `PRAXIS_RESPONSE_LANGUAGE` as its own line when set — issues #472, #1339, #1476 |
| [response-language-nudge](../../hooks/advisory-nudge/response-language-nudge/spec.md) | PostToolUse (`.*`) | Opt-in (`PRAXIS_RESPONSE_LANGUAGE`, Korean values only): nudge via `additionalContext` when the current turn's last assistant narration falls below a 0.3 Hangul-character ratio after stripping code/URLs/paths/identifiers; per-message dedup — issue #1476 |
...
| [second-failure-advisory](../../hooks/postuse-correction/second-failure-advisory/spec.md) | PostToolUseFailure (claude only, issue #1337) | Advisory on a repeated `(tool_name, signature)` failure pattern's second occurrence in the same session to reduce unbounded re-try loops; the failure event is what lets a failed Bash command count at all. The `PostToolUse` registration it ran alongside for one release was removed once that run ended |
...
`negative-existence-verdict-gate` → `bypass-route-signal` →
...
`prose-option-menu-advisory` → `early-stop-advisory` →
...
| [bypass-route-signal](../../hooks/completion-verify/bypass-route-signal/spec.md) | Stop | Observe-only: append one `bypass-route-events-*` record when the final message pairs a bypass-route noun (권한 규칙/allow list/settings.json/disableAllHooks/guarded path) with a proposal frame in the same paragraph, after the gate's own `Bypass (if truly needed):` relay lines are stripped — a frequency meter for `RULE-BACKSTOP-GAPS` gap #4's prose lane, never blocks and never advises (#1338) |
...
| [pr-anchor-existence-gate](../../hooks/completion-verify/pr-anchor-existence-gate/spec.md) | Stop | Advisory on the 1st Stop, block on the 2nd+ when a successful non-draft `gh pr create` this session received no verification-anchor post (`gh pr comment` / write `gh api .../{issues,pulls}/<N>/comments`) — existence only, not the anchor's shape (that's `anchor-comment-gate`); bypass `PRAXIS_PR_ANCHOR_BYPASS`, advisory cap `PRAXIS_PR_ANCHOR_ADVISORY=<N>` / `=forever` (#1113, #1443) |
...
| [early-stop-advisory](../../hooks/completion-verify/early-stop-advisory/spec.md) | Stop | Advisory (a user-facing `systemMessage`; the model does not receive it) in every session, or with the unattended-run marker `PRAXIS_UNATTENDED=1` a Stop block whose reason asks the model to continue or name the blocker (at most 2 per human turn, then the notice), when the last assistant text ends the turn with requested work still open — a closing next-step announcement with no tool call (`다음 단계로 …하겠습니다` / `Next, I'll …`), a prose offer to continue on the user's preference (`원하시면 남은 …도 이어서` / `if you'd like, I can continue`), or an interim report listing unfinished items; silent on a blocker stated as a need or lack, a user-requested report/plan/question, `stop_hook_active` (notice mode only), and type-3 menus owned by `prose-option-menu-advisory`; bypass `PRAXIS_EARLY_STOP_BYPASS` (#1498) |
