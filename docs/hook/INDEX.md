...
| [model-routing-advisory](../../hooks/advisory-nudge/model-routing-advisory/spec.md) | PreToolUse | Advisory nudge when a Bash delegation's `--model <tier>` mismatches the tier implied by task keywords (`find → haiku`, `implement → sonnet`, `architect/security → opus`); replaces the always-loaded `Skill & Agent Routing` + `Model Routing Rules` blocks, whose full tree now lives in this hook's spec — issue #786 |
