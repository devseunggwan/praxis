...
| [proposal-premise-gate](../../hooks/completion-verify/proposal-premise-gate/spec.md) | Stop | Advisory when a prose proposal block rests on code-checkable premises that were never probed in the session — prose proposals have no PreToolUse surface, so the Stop lane is the only firing point (#846) |
