...
| `debt` | `praxis:debt`, `debt ledger`, `지연 결정`, `deferred decision`, `기술 부채 원장`, `commit trailer audit`, `나중에`, `영원히 안 함` | To harvest commit-trailer and compounding-comment deferred-decision markers into a report-only ledger | `/praxis:debt` |
...
| `cmux-resume-sessions` | `resume sessions`, `session resume`, `session restore`, `cmux resume`, `restore from snapshot`, `rehydrate sessions`, `세션 복원`, `스냅샷 복구`, `스냅샷 복원`, `살려야` | To restore workspaces from a saved snapshot (for crash recovery, use `cmux-recover-sessions`) | `/praxis:cmux-resume-sessions` |
