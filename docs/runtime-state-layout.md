...
| `stop-scan-<hook>-<sid>.json`                                      | Stop gates' resumable whole-session reduction cursors (`_transcript.reduce_transcript_resumable`, #1237) |
| `scan-<hook>-<part>-<sid>.json`                                    | Resumable scan cursors of `block-commit-without-codex-review`, `unenforced-step-advisory`, `rejected-mutation-reconsent-gate` and `retrospect-mix-check` (`_transcript.scan_transcript_resumable`; `<part>` is the transcript file or trigger the cursor follows, #1277 / #1278 / #1280) |
