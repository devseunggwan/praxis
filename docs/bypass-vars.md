...
| `PRAXIS_DENIED_ACTION_STRICT` | `denied-action-report-gate` | Exact value `1` after stripping. The advisory tier reaches the user but not the model, so this is the tier that makes the finding actionable in-session (#1265) |
...
| `PRAXIS_PIPEFAIL_ADVISORY_REWRITE` | `pipefail-advisory` | Exact value `1` after stripping. Prepends `set -o pipefail; ` to a mutating command piped into a truncating sink. The `&&` masked-gating finding is out of scope — its own first remedy is splitting the chain, which no rewrite can express |
